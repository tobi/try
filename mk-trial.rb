...
class TrialSelector
  TRIAL_DIR = ENV['TRIAL_DIR'] || File.expand_path("~/src/trials")
  ...
  def initialize(search_term = "")
    ...
    FileUtils.mkdir_p(TRIAL_DIR) unless Dir.exist?(TRIAL_DIR)
  end
  ...
  def load_all_trials
    ...
    @all_trials ||= begin
      trials = []
      Dir.foreach(TRIAL_DIR) do |entry|
        next if entry == '.' || entry == '..'

        path = File.join(TRIAL_DIR, entry)
        stat = File.stat(path)
  ...
  def get_trials
    ...
    scored_trials = @all_trials.map do |trial|
      score = calculate_score(trial[:basename], @input_buffer, trial[:ctime], trial[:mtime])
      trial.merge(score: score)
    end

    # Filter only if searching, otherwise show all
    if @input_buffer.empty?
      scored_trials.sort_by { |t| -t[:score] }
    else
      # When searching, only show matches
      filtered = scored_trials.select { |t| t[:score] > 0 }
  ...
  def main_loop
    loop do
      trials = get_trials
      total_items = trials.length + 1  # +1 for "Create new" option
      ...
      when "\e[D"  # Left arrow - ignore
        # Do nothing
      when "\r", "\n"  # Enter
        if @cursor_pos < trials.length
          handle_selection(trials[@cursor_pos])
        else
          # Selected "Create new"
          handle_create_new
        end
  ...
  def render(trials)
    ...
    # Header
    print "\e[1;36m📁 Trial Directory Selection\e[0m\r\n"
    ...
    (@scroll_offset...visible_end).each do |idx|
      ...
      if idx == trials.length && trials.any? && idx >= @scroll_offset
        print "\r\n"
      end
      ...
      # Display trial or "Create new" option
      if idx < trials.length
        ...
        if trial[:basename] =~ /^(\d{4}-\d{2}-\d{2})-(.+)$/
          ...
          if !@input_buffer.empty?
            print highlight_matches_for_selection(trial[:basename], @input_buffer, is_selected)
          else
            if is_selected
              print "\e[97m#{trial[:basename]}\e[39m"  # Bright white on selection
            else
              print trial[:basename]
            end
          end
        ...
        time_text = format_relative_time(trial[:mtime])
        score_text = sprintf("%.1f", trial[:score])
  ...
  def handle_selection(trial)
    # Select existing trial
    @selected = { type: :cd, path: trial[:path] }
  end
  ...
  def handle_create_new
    ...
    if !@input_buffer.empty?
      ...
      full_path = File.join(TRIAL_DIR, final_name)
...
if __FILE__ == $0
  ...
  if ARGV.include?('--help') || ARGV.include?('-h')
    puts <<~HELP
      \e[1;36mtrial - Lightweight experiment directory manager\e[0m

      \e[1;33mUsage:\e[0m
        trial [search_term]     Interactive directory selector
        trial --help           Show this help message
        trial --eval           Output shell function for eval

      \e[1;33mDescription:\e[0m
        Create and navigate to trial/experiment directories with ease.
      ...
      \e[1;33mShell Integration:\e[0m
        ...
        Then use: \e[1;32mtrial [search_term]\e[0m

      \e[1;33mExamples:\e[0m
        trial              # Browse all experiments
        trial redis        # Find/create Redis-related experiment
        trial new api      # Start with "new-api" as suggestion

      \e[1;33mDirectory Location:\e[0m
        All trials are stored in: \e[90m~/src/trials/\e[0m
    HELP
    exit 0
    ...
    selector = TrialSelector.new(search_term)
    result = selector.run
